...
# Define input and output slot classes for KarrasScheduler
class KarrasSchedulerInputs(InputSlots):
    steps: Slot[int]
...
class KarrasSchedulerOutputs(OutputSlots):
    sigmas: Slot[Any]
    def __init__(self, node: "Node"):
...
class KarrasScheduler(Node[KarrasSchedulerInputs, KarrasSchedulerOutputs]):
    ...
    def __init__(self, steps: int = 20, sigma_max: float = 14.614642, sigma_min: float = 0.0291675, rho: float = 7.0):
        super().__init__(**{"steps": steps, "sigma_max": sigma_max, "sigma_min": sigma_min, "rho": rho})
        self.inputs = KarrasSchedulerInputs(self)
        self.outputs = KarrasSchedulerOutputs(self)
