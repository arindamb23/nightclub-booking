
# This file is auto-generated by generate_nodes.py
# Do not edit this file directly.

from ..base_node import Node, InputSlots, OutputSlots, Slot, Model, Conditioning, Latent, Image, Vae, Clip
from typing import Dict, Any, List, Union, Literal, Optional, Union

# Define input and output slot classes for RecraftStyleV3LogoRaster
class RecraftStyleV3LogoRasterInputs(InputSlots):
    substyle: Slot[Union[Literal['emblem_graffiti', 'emblem_pop_art', 'emblem_punk', 'emblem_stamp', 'emblem_vintage'], str]]
    def __init__(self, node: "Node"):
        self.substyle = Slot[Union[Literal['emblem_graffiti', 'emblem_pop_art', 'emblem_punk', 'emblem_stamp', 'emblem_vintage'], str]](node, "substyle", ['emblem_graffiti', 'emblem_pop_art', 'emblem_punk', 'emblem_stamp', 'emblem_vintage'])

class RecraftStyleV3LogoRasterOutputs(OutputSlots):
    recraft_style: Slot[Any]
    def __init__(self, node: "Node"):
        self.recraft_style = Slot[Any](node, "recraft_style", 'RECRAFT_V3_STYLE')

class RecraftStyleV3LogoRaster(Node[RecraftStyleV3LogoRasterInputs, RecraftStyleV3LogoRasterOutputs]):
    """
    Original name: RecraftStyleV3LogoRaster
    Category: api node/image/Recraft
    Select realistic_image style and optional substyle.

    Inputs:
        - substyle (Union[Literal['emblem_graffiti', 'emblem_pop_art', 'emblem_punk', 'emblem_stamp', 'emblem_vintage', str)

    Outputs:
        - recraft_style (Any)
    """
    _original_name: str = 'RecraftStyleV3LogoRaster'

    def __init__(self, substyle: Union[Literal['emblem_graffiti', 'emblem_pop_art', 'emblem_punk', 'emblem_stamp', 'emblem_vintage'], str]):
        super().__init__(**{"substyle": substyle})
        self.inputs = RecraftStyleV3LogoRasterInputs(self)
        self.outputs = RecraftStyleV3LogoRasterOutputs(self)
