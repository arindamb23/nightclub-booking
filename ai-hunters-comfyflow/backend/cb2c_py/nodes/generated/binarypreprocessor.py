
# This file is auto-generated by generate_nodes.py
# Do not edit this file directly.

from ..base_node import Node, InputSlots, OutputSlots, Slot, Model, Conditioning, Latent, Image, Vae, Clip
from typing import Dict, Any, List, Union, Literal, Optional, Union

# Define input and output slot classes for BinaryPreprocessor
class BinaryPreprocessorInputs(InputSlots):
    image: Slot[Image]
    bin_threshold: Slot[int]
    resolution: Slot[int]
    def __init__(self, node: "Node"):
        self.image = Slot[Image](node, "image", 'IMAGE')
        self.bin_threshold = Slot[int](node, "bin_threshold", 'INT')
        self.resolution = Slot[int](node, "resolution", 'INT')

class BinaryPreprocessorOutputs(OutputSlots):
    image: Slot[Image]
    def __init__(self, node: "Node"):
        self.image = Slot[Image](node, "IMAGE", 'IMAGE')

class BinaryPreprocessor(Node[BinaryPreprocessorInputs, BinaryPreprocessorOutputs]):
    """
    Original name: BinaryPreprocessor
    Category: ControlNet Preprocessors/Line Extractors
    

    Inputs:
        - image (Image)
        - bin_threshold (int) (default: 100)
        - resolution (int) (default: 512)

    Outputs:
        - image (Image)
    """
    _original_name: str = 'BinaryPreprocessor'

    def __init__(self, image: Union[Slot[Image], Image], bin_threshold: int = 100, resolution: int = 512):
        super().__init__(**{"image": image, "bin_threshold": bin_threshold, "resolution": resolution})
        self.inputs = BinaryPreprocessorInputs(self)
        self.outputs = BinaryPreprocessorOutputs(self)
